...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$default_group = $this->where('name', $this->ion_auth_model->getSet('default_group')->value)->group()->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$getLevel = $this->userLevel($user->id)->row();
				$level = $getLevel->level;
				$this->set_session($user,$level);
	...
	public function userLevel($id){
		return $this->db->query("SELECT * FROM groups INNER JOIN users_groups ON groups.id = users_groups.group_id where users_groups.user_id = $id ");
	}
	...
	public function is_max_login_attempts_exceeded($identity) {
		if ($this->ion_auth_model->getSet('track_login_attempts')->value) {
			$max_attempts = $this->ion_auth_model->getSet('maximum_login_attempts')->value;
			if ($max_attempts > 0) {
	...
	public function num_row_user(){
		return $this->db->count_all_results('users'); 
	}
	
	public function num_row_groups(){
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description,'.$this->tables['groups'].'.level')
	...
	{		
		...
		$userRow 	 = $this->user($id)->row();
		$oldUsername = $userRow->username;
		$userRow 	 = $this->user($id)->row();
		$oldEmail	 = $userRow->email;
	
		if ($oldUsername != $data['username'] || $oldEmail != $data['email'])
		{
			$control['username'] = false;
			$control['email'] = false;
			if ($oldUsername != $data['username'] && $this->username_check($data['username']))
			{
				$this->set_error('account_creation_duplicate_username');
				$control['username'] = true;
			}
			if ($oldEmail != $data['email'] && $this->email_check($data['email']))
			{
				$this->set_error('account_creation_duplicate_email');
				$control['email'] = true;
			}
			if ($control['username'] == TRUE || $control['email'] == TRUE)
			{
				return false;
			}
		}
	...
	**/
	 ...
	 * @author jrmadsen67
	 **/
	public function set_session($user, $level = NULL)
	{
	 ...
	 * @author aditya menon
	*/
	public function create_group($group_name = FALSE, $group_description = '', $level = 1, $additional_data = array())
	{
		...
		$data = array('name'=>$group_name,'description'=>$group_description,'level'=>$level);
	 ...
	 * @author aditya menon
	 **/
	public function update_group($group_id = FALSE, $group_name = FALSE, $level = 1, $additional_data = array())
	{
	...
	**/
	...
	protected function _call_hook($event, $name)
	{
        ...
        if (isset($this->_ion_hooks->{$event}[$name]) && method_exists($this->_ion_hooks->{$event}[$name]->class, $this->_ion_hooks->{$event}[$name]->method))
		{
            ...
            //\application\helpers\Generic::_setTrace($hook,1);
